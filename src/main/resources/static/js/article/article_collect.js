var vm = new Vue({
    ...
    methods: {
        ...
        search: function (page) {
            axios({
                url: `/article/searchCollect/${page}`,
                params: {title: this.title, uid: this.loginUser.id}
            }).then(response => {
                ...
                if (response.data.flag) {
                    ...
                    // 当前页数据
                    this.collectAList = response.data.data.pageResult.rows;
                    ...
                    if (this.collectAList.length == 0 && page> this.totalPage) {
                        this.search(1);
                    }
